const ActiveApp = ({ output }) => {
  ...
  function getAppIcon(appId) {
    if (/spotify/i.test(appId)) {
      ...
      /firefox/i.test(appId) ||
      /zen browser|zenbrowser|zen/i.test(appId)
    ) {
      return <i className="nf nf-seti-firefox"></i>;
    ...
    } else if (/teams|microsoft teams/i.test(appId)) {
      return <i className="nf nf-md-microsoft_teams"></i>;
  ...
  if (!output.glazewm || !output.glazewm.focusedWorkspace) return null;
